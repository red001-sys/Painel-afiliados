...
class _SplashScreenState extends State<SplashScreen>
  ...
  Future<void> _navigate() async {
    ...
    // Se a URL indica um link de redefinição de senha, navega direto pra
    // tela de redefinir senha. Sem essa checagem, a sessão de recuperação
    // (que também conta como "sessão existe") faz a splash mandar o usuário
    // pro dashboard/admin por cima da navegação de redefinição.
    if (Uri.base.toString().contains('type=recovery')) {
      debugPrint('[SPLASH] Recovery link detected in URL → navigating to reset password');
      if (!mounted) return;
